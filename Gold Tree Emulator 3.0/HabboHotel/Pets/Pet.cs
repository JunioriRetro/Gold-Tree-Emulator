...
namespace GoldTree.HabboHotel.Pets
{
	internal sealed class Pet
	{
		...
		public Room Room
		{
			get
			{
				if (!IsInRoom)
				{
					...
					return GoldTree.GetGame().GetRoomManager().GetRoom(RoomId);
			}
		}
		...
		public int Age
		{
			get
			{
				return (int)Math.Floor((GoldTree.GetUnixTimestamp() - this.CreationStamp) / 86400.0);
			}
		}
		public string Look
		{
		...
		public string OwnerName
		{
			get
			{
				return GoldTree.GetGame().GetClientManager().GetNameById(OwnerId);
			}
		}
